...
def _row_to_detail(row: list) -> TemplateDetail:
    # row: [id, name, config_json, created_at, updated_at]
...
def save_template(name: str, config: TemplateConfig) -> TemplateDetail:
    """
    ...
    if rows:
        ...
        template_id = execute_returning_scalar(
            "INSERT INTO dbo.report_templates (name, config) "
            "OUTPUT INSERTED.id "
            "VALUES (?, ?)",
            [name, config_json],
        )
